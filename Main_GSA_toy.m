...
mc_iter = 4000;        % Monte Carlo iterations for statistical analysis (put more than 1 if you want to carry out some statistical analysis via Monte Carlo simulation)
...
for t = 1:1    %%% --> change this if you want just to focus on a specific time step
    ...
    S.br1.meas.index = [1:branch.num]';                % indication of the branches where active and reactive power meas are placed (measurement on the starting side of the branch).
    ...
    V.mag.meas.unc = [1,3,3,3,3,3,1,3,3,3]';           % uncertainty (in percent) of voltage magnitude measurements
    S.inj.meas.unc = 50;           % uncertainty (in percent) of active and reactive power injection measurements (pseudo-measurements)
    S.br1.meas.unc = [50;50;50;50;50;50;50;1;50];           % uncertainty (in percent) of active and reactive branch power measurements
    ...
    %Create zdata samples
    %         load('zdata_samples.mat')
    %% Sample creation
    ...
    XX = zdata_samples'; %Create X matrix for PCE
    Ypce=squeeze(Vtime_est)'; %Create Y matrix for PCE
    %% UQLAB
    uqlab %Activate uqlab
    input_params=[zdata(:,2) zdata(:,6)]; %Select mean and std of each of the inputs considered in the PCE
    ...
    %% Heatmaps creation
    figure(),h1=heatmap(first_order,'CellLabelColor','none'); %First order heatmap
    ...
    for ii=1:size(zdata,1)
        switch zdata(ii,1)
            ...
            case 4
                h1.YDisplayLabels{ii,1}=['P',num2str(zdata(ii,4)),'-',num2str(zdata(ii,5))];
            case 5
                h1.YDisplayLabels{ii,1}=['Q',num2str(zdata(ii,4)),'-',num2str(zdata(ii,5))];
        end
    end
    h1.Colormap=parula;
    h1.Title='First order indexes';

    figure(),ht=heatmap(total_order,'CellLabelColor','none'); %Total order heatmap
    xlabel('Voltage magnitude')
    ylabel('Measured value')
...
%% Rankings
Vindex= [1,7]; %Vector containing the indexes of existing voltage meters
Sbrindex= [8]; %Vector containing the indexes of existing powerflow meters
stdV_mc=UAcode_test(Vindex,Sbrindex); %Compute extended uncertainty profile
%First metric
rank1=total_order*(stdV_mc.^2); %compute SI*unc^2
rank1=[ht.YDisplayLabels num2cell(rank1)];
rank1=sortrows(rank1,2,'descend');
first_order_cell=num2cell(first_order);
