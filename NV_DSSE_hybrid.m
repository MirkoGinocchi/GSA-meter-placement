function[V, I, S, num_iter] = NV_DSSE_hybrid(branch, node, V, zdata, Y, Bs)
...
pfstart = qistart + nqi;                                                   % Index of start of Real Power Flow measurements..
qfstart = pfstart + npf;                                                   % Index of start of Reactive Power Flow measurements..
iampfstart = qfstart + nqf;                                                % Index of start of Current measurements..
...
for i=1:nsynciamp                % calcolo della matrice di covarianza associata alle correnti reale e immaginaria, secondo legge propagazione incertezza
...
Cov = sparse(hidx, vidx, covariances);                                       % definizione della matrice sparsa della covarianza tramite assegnazione di indice di colonna, riga, e relativo valore
Cov = full(Cov);
W = Cov\eye(size(Cov));                                                       % inverso matrice covarianza = matrice dei pesi
W = sparse(W);
State = [V.real; V.imag];
...
while (epsilon > 10^-6 && num_iter <= 100)
    ...
    %%% 2) Power Injection Measurements
    h2 = H2*State;
    h3 = H3*State;
